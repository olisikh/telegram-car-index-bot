...
const timingFeedback = (timings: RecognitionTimings | undefined): string => {
  if (!timings) return "";
  const lines = [
    timings.detectionMs === undefined ? undefined : `🕵️‍♂️ Пошук: ${stageElapsed(timings.detectionMs)}`,
    timings.croppingMs === undefined ? undefined : `✂️ Обрізання: ${stageElapsed(timings.croppingMs)}`,
    timings.ocrMs === undefined ? undefined : `👁️ OCR: ${stageElapsed(timings.ocrMs)}`,
