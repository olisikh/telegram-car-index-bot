...
Bot replies default to English. `/lang en` and `/lang uk` persist the selected language independently for each chat and update that chat's command menu; `/lang ua` is accepted as a compatibility alias for Ukrainian. User-facing text lives in the typed `src/i18n.ts` catalog rather than command handlers.
