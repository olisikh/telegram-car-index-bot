...
English is used by default. Run `/lang uk` for Ukrainian or `/lang en` to switch back. The choice and command menu are saved for that chat. `/lang ua` also selects Ukrainian for compatibility.
